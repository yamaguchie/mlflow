...
class DefaultExperimentProvider(object):
    ...
    When an experiment is created via the fluent ``mlflow.start_run`` method, MLflow iterates through all
    registered DefaultExperimentProvider. For each context provider where ``in_context`` returns ``True``,
    MLflow calls the ``get_experiment_id`` method on the context provider to compute experiment_id for the experiment.
    """
